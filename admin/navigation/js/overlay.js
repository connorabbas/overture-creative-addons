/* (function () {
    'use strict';
    jQuery('.hamburger-menu').click(function (e) {
        e.preventDefault();
        if (jQuery(this).hasClass('active')) {
            jQuery(this).removeClass('active');
            jQuery('.menu-overlay').fadeToggle('fast', 'swing');
            jQuery('.menu .menu-list').slideToggle('slow', 'swing');
            jQuery('#mobile-menu-dd').delay(250).fadeToggle('fast', 'swing');
        } else {
            jQuery(this).addClass('active');
            jQuery('.menu-overlay').fadeToggle('fast', 'swing');
            jQuery('.menu .menu-list').slideToggle('slow', 'swing');
            jQuery('#mobile-menu-dd').delay(350).fadeToggle('fast', 'swing'); 
        }
    })
})(); */


jQuery('.hamburger-menu').click(function (e) {
    e.preventDefault();
    if (jQuery(this).hasClass('active')) {
       /*  jQuery('#nav-toggle-button').css('position', 'absolute');
        jQuery('#nav-toggle-button').css('top', '0'); */
        //jQuery(this).removeClass('active');
        //jQuery(this).fadeToggle('slow', 'linear');
        jQuery('#mobile-menu-dd').fadeToggle('fast', 'swing');
        jQuery('#overlay-ham-btn-outer').fadeToggle('fast', 'swing');
        closeNav();
    } else {
        jQuery(this).fadeToggle('fast', 'swing');
        /* jQuery('#nav-toggle-button').css('position', 'fixed');
        jQuery('#nav-toggle-button').css('top', 'auto'); */
        //jQuery(this).addClass('active');
        //jQuery(this).fadeToggle('fast', 'swing');
        jQuery('#mobile-menu-dd').fadeToggle('slow', 'swing'); 
        jQuery('#overlay-ham-btn-outer').fadeToggle('slow', 'swing');
        openNav();
    }
});

jQuery('#overlay-ham-btn-outer').click(function (e) {
    jQuery('#def-ham-btn').fadeToggle('slow', 'linear');
});


jQuery('#primary-mobile-nav-menu li.menu-item-has-children').each(function(){
    jQuery(this).append('<div class="abs-mobile-icon"><i class="mobile-dd-icon fas fa-chevron-down"></i></div>');
});

jQuery('#primary-mobile-nav-menu .abs-mobile-icon').click(function (event) {
    console.log('clicked');
    jQuery(this).prev().slideToggle( 'fast', 'swing' );
    jQuery(this).find('.mobile-dd-icon').toggleClass("rot");
});

function openNav() {
    document.getElementById("myNav").style.height = "100%";
}

function closeNav() {
    document.getElementById("myNav").style.height = "0%";
}